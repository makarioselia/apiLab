...
namespace EShop.Manegers
{
    public class AccountManager:BaseManager<User>
    {
        ...
        {
            //if correct Email
            var User = await UserManager.FindByEmailAsync(vmodel.UserTryLoginName);
            if(User != null)
             return await  signInManager.PasswordSignInAsync(User, vmodel.Password, true, true);
            else
             return await  signInManager.PasswordSignInAsync(vmodel.UserTryLoginName, vmodel.Password, true, true);
        }
        ...
        public async Task<User> FindByEmail (string emailAccount)
        {
            return await UserManager.FindByEmailAsync(emailAccount);
        }
